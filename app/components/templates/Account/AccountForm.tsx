...
export default function AccountForm({ userInfo, token, getUserInfo }: AccountFormProps) {
    ...
    const updateUser = async (user: {}) => {
        ...
        setLoading(true)
        try {
            const res = await fetch(`http://localhost:3000/api/users/update/${token}`, {
                method: "PUT",
                        ...
                        {({ errors, touched }) => (
                            <Form className='xl:min-w-[600px]'>
                                ...
                                <Field name="username" className='border outline-none w-full p-3 mb-3 text-gray-500'
                                    ...
                                    type="text" />
                                {errors.name && touched.name ? <p className='text-xs py-2 text-rose-400'>{errors.name}</p> : null}
                                <Field name="name" className='border outline-none w-full p-3 mb-3 text-gray-500'
                                    placeholder={userInfo?.name || 'Name...'}
                                    type="text" />
                                {errors.phone && touched.phone ? <p className='text-xs py-2 text-rose-400'>{errors.phone}</p> : null}
                                <Field name="phone" className='border outline-none w-full p-3 mb-3 text-gray-500'
                                    placeholder={userInfo?.phone || 'Mobile Number...'}
                                    type="text" />
                                {errors.age && touched.age ? <p className='text-xs py-2 text-rose-400'>{errors.age}</p> : null}
                                <Field name="age" className='border outline-none w-full p-3 mb-3 text-gray-500'
                                ...
                                {errors.location && touched.location ? <p className='text-xs py-2 text-rose-400'>{errors.location}</p> : null}
                                <Field name="location" className='border outline-none w-full p-3 mb-3 text-gray-500'
                                    placeholder={userInfo?.location || 'Address...'}
                                    type="text" />
                                <div className='text-right'>
                                    {loading ? (
                                        <button  className='btn' disabled><span className='animate-pulse'>Loading...</span></button>
                                    ) : (
